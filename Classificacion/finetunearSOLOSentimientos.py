...
def getSentimentForInstance(instance, classifier):
    truncated_instance = truncate_text(instance)
    prediction = classifier(truncated_instance, )[0]
    res = ''
    for emotion in prediction:
        if emotion['score'] >= 0.8: res = res + ' ' + f'{emotion["label"]}: VERY HIGH;'
        elif 0.6 <= emotion['score'] < 0.8: res = res + ' ' + f'{emotion["label"]}: HIGH;'
        elif 0.4 <= emotion['score'] < 0.6: res = res + ' ' + f'{emotion["label"]}: NORMAL;'
        elif 0.2 <= emotion['score'] < 0.4: res = res + ' ' + f'{emotion["label"]}: LOW;'
        else: res = res + ' ' + f'{emotion["label"]}: VERY LOW;'
    return f'{res}'
...
dataToWrite = "./results/new/"

data = pd.read_csv(f'../Datasets/Suicide_Detection_10000.csv')
...
dataset_dict = {}
dataset_dict['train'] = {}
dataset_dict['train']['text'] = [f'{getSentimentForInstance(text, classifier)}' for text in train['text']]
...
dataset_dict['val'] = {}
dataset_dict['val']['text'] = [f'{getSentimentForInstance(text, classifier)}' for text in val['text']]
...
dataset_dict['test'] = {}
dataset_dict['test']['text'] = [f'{getSentimentForInstance(text, classifier)}' for text in test['text']]
...
trainer.save_model(dataToWrite + 'best_model_SOLOSENTIMIENTOS2') # save best model
